...
---insert resume
INSERT INTO resume_data (creation_time, expire_time, user_id, disabled, visible, area_id)
SELECT
    now()-(random() * 365 * 24 * 3600 * 5) * '1 second'::interval AS creation_time,
    now()-(random() * 365 * 24 * 3600 * 5) * '1 second'::interval AS expire_time,
    (random() * 1000000)::int AS employer_id,
...
INSERT INTO resume_data_specialization(resume_data_specialization_id, specialization_id)
SELECT
  (SELECT resume_id FROM resume_data WHERE resume_id = i) AS resume_body_id,
  (SELECT ((random() * 49)::int  + i % 2 + 1)::integer) AS specialization_id
FROM generate_series(1, (SELECT count(resume_body_id) FROM resume_data_body)) AS g(i);


INSERT INTO vacancy_body_specialization (vacancy_body_specialization_id, specialization_id)
SELECT
  (SELECT vacancy_id FROM vacancy WHERE vacancy_id = i) AS vacancy_body_specialization_id,
  (SELECT ((random() * 49)::int  + i % 2 + 1)::integer) AS specialization_id
